...
having count(nzp_kvar)>1";
        ...
        public int InsertKart(int nzp_gil, string nzp_kvar, string fam, string ima, string otch, string dat_rog, string gender, int nzp_dok, string serij, string nomer, string vid_dat, string vid_mes, string tprp, string dat_reg, int nzp_rod, string rodstvo)
        {
            string connStr = "Server=192.168.1.25;Database=billAuk;User ID=postgres;Password=Admin;CommandTimeout=180000;";
            if (tprp.Length != 1)
            {
            ...
            string cmdText = "INSERT INTO bill01_data.kart(nzp_gil, isactual, fam, ima, otch, dat_rog, gender, tprp, nzp_bank, nzp_user, nzp_tkrt, nzp_kvar, nzp_nat, nzp_rod, nzp_dok, serij, nomer, vid_mes, vid_dat, dat_sost, dat_ofor, dat_izm, is_unl, cur_unl, rodstvo) " +
                "VALUES("+nzp_gil+", 1, '"+fam+"', '"+ima+"', '"+otch+"', to_date('"+dat_rog+"', 'dd.mm.yyyy'), '"+gender+"', '"+tprp+"', 1, 1, 1, "+nzp_kvar+", -1, "+nzp_rod
                +", "+nzp_dok+", '"+serij+"', '"+nomer+"', '"+vid_mes+"', to_date('"+vid_dat+"', 'dd.mm.yyyy'),  to_date('"+dat_reg+"', 'dd.mm.yyyy'),  to_date('"+dat_reg
                +"', 'dd.mm.yyyy'), current_date, 1, 1, '"+rodstvo+"') returning nzp_kart";
            NpgsqlConnection conn = new NpgsqlConnection(connStr);
            NpgsqlCommand cmd = new NpgsqlCommand(cmdText, conn);
            conn.Open();
            //Получаем nzp_prm добавленной нами записи
